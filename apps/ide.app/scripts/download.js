...
async function downloadExtension(url, targetDirName) {
...
function unzipFile(extensionDir, tmpZipFile) {
...
const installExtension = async (namespace, name, version) => {
    const extPath = version
        ? `${namespace}/${name}/${version}`
        : `${namespace}/${name}`;
    const res = await urllib.request(`${api}${extPath}`, {
        dataType: 'json',
        timeout: 100000,
    });
    if (res.data.files && res.data.files.download) {
        const targetDirName = path.basename(`${namespace}.${name}`);
        const extensionDir = path.join(targetDir, targetDirName);

        console.log(extensionDir);
        if (!(await fs.exists(extensionDir))) {
            const {tmpZipFile} = await downloadExtension(
                res.data.files.download, targetDirName
            );
            // 解压插件

            await unzipFile(extensionDir, tmpZipFile);
            rimraf.sync(tmpZipFile);
        } else {
            console.log(`跳过${targetDirName}`)
        }
    }
};
