...
const DebugParamsEditor = () => {
  const editorService = useInjectable<EditorCollectionService>(
    EditorCollectionService
  );
  const documentService = useInjectable<IEditorDocumentModelService>(
  ...
  const bindDebugParamsFile = async () => {
    if (paramEditorRef.current) {
      const editor = editorService.createCodeEditor(
      ...
      const uri = new URI((await workspaceService.roots)?.[0]?.uri).resolve(
        './debug_params.json'
      );
...
export class DebugContribution implements ComponentContribution {
  registerComponent(registry: ComponentRegistry) {
    const debugComponentRegistryInfo = registry.getComponentRegistryInfo(
      '@opensumi/ide-debug'
    );
    if (debugComponentRegistryInfo?.views) {
      debugComponentRegistryInfo.views =
        debugComponentRegistryInfo?.views.filter(
          view => view.id !== 'debug-welcome'
        );
